...
- **Configurazione Gara**: Admin page at `/configurazione-gara` (`client/src/pages/ConfigurazioneGara.tsx`) for per-month gara configuration. Hook `client/src/hooks/useGaraConfig.ts` wraps all `/api/gara-config*` endpoints. Three-tab layout: **PDV** (PDV list with cluster/calendar editing, tipo gara selector gara_operatore/gara_operatore_rs, modalità RS per_pdv/per_rs), **Soglie** (editable Mobile S1-S4 + forecast, Fisso S1-S5 + forecast, Partnership Reward target100/premio100 per PDV or per RS with key-based mapping), **Energia & Ass.** (aggregate Energia config: pdvInGara, targetNoMalus, targetS1-S3, pistaSoglia S1-S5; Assicurazioni config: pdvInGara, targetNoMalus, targetS1-S2). Import from simulator, manual PDV add, save, history browser. Soglie auto-initialized from cluster using existing helper functions. Admin/super_admin only.

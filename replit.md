...
- **Configurazione Gara**: Admin page at `/configurazione-gara` (`client/src/pages/ConfigurazioneGara.tsx`) for per-month gara configuration. Hook `client/src/hooks/useGaraConfig.ts` wraps all `/api/gara-config*` endpoints. Features: month/year selector, PDV list with cluster/calendar editing, import from simulator (pdv_configurations or organization_config), manual PDV add, save, history browser. Admin/super_admin only.
...
- **Navigation Structure**: AppNavbar groups links into two dropdown sections — **Gara** (Dashboard, Configurazione, Vendite BiSuite, Mappatura) and **Simulatore** (Preventivatore, Dashboard Sim., Tabelle Calcolo). Admin items (Super Admin, Gestione Team) appear as standalone buttons before the dropdowns. Mobile menu uses section labels with separators.

...
export function calcolaProtecta(
  ...
  return puntiVendita.map(pdv => {
    ...
    const dettaglioProdotti = (Object.keys(PROTECTA_GETTONI) as ProtectaProduct[])
      .map(key => {
        const pezzi = attivato[key];
        const gettone = PROTECTA_GETTONI[key];
        return {
    ...
    const pezziTotali = (Object.keys(PROTECTA_GETTONI) as ProtectaProduct[])

...
export function buildHardcodedDefaults(): TabelleCalcoloConfig {
  ...
  return {
    ...
    fisso: { soglieCluster: { ...FISSO_SOGLIE_DEFAULTS }, puntiPerPezzo, euroPerPezzo, gettoniContrattuali: { ...GETTONI_DEFAULTS } },
          ...
          <table className="w-full text-sm min-w-[500px]" data-testid="table-gara-mobile-soglie">
            <thead>
              <tr className="bg-primary text-primary-foreground">
                <th className="p-2 text-left font-medium rounded-tl-md sticky left-0 bg-primary z-20">Cluster</th>
            ...
            <tbody>
              {soglieKeys.map(key => (
                <tr key={key} className="even:bg-muted/30">
                  <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{MOBILE_SOGLIE_LABELS[key]}</td>
                  {[0, 1, 2, 3].map(i => {
      ...
      <Card>
        ...
        <CardContent><ScrollableTable>
          <table className="w-full text-sm" data-testid="table-gara-mobile-punti">
            <thead>
              <tr className="bg-primary text-primary-foreground">
                <th className="p-2 text-left font-medium rounded-tl-md sticky left-0 bg-primary z-20">Tipo Attivazione</th>
      ...
      <Card>
        ...
        <CardContent><ScrollableTable>
          <table className="w-full text-sm min-w-[500px]" data-testid="table-gara-mobile-moltiplicatori">
            <thead>
              <tr className="bg-primary text-primary-foreground">
                <th className="p-2 text-left font-medium rounded-tl-md sticky left-0 bg-primary z-20">Gruppo</th>
            ...
            <tbody>
              {moltiplicatoriKeys.map(key => (
                <tr key={key} className="even:bg-muted/30">
                  <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{MOLTIPLICATORI_LABELS[key]}</td>
                  {[0, 1, 2, 3].map(i => {
  ...
  return (
    <>
      ...
      <Card>
        ...
        <CardContent><ScrollableTable>
          <table className="w-full text-sm min-w-[600px]" data-testid="table-gara-fisso-punti-euro">
            <thead>
              <tr className="bg-primary text-primary-foreground">
            ...
            <tbody>
              {fissoCategories.map(cat => {
                ...
                const puntiDef = getNestedValue(baseDefaults, puntiPath) ?? cat.puntiPerPezzo;
                const euroVal = getNestedValue(config, euroPath) ?? cat.euroPerPezzo;
                const euroDef = getNestedValue(baseDefaults, euroPath) ?? cat.euroPerPezzo;
                return (
                  <tr key={cat.type} className="even:bg-muted/30">
                    <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{cat.label}</td>
                    <td
                      className="p-2 text-center border border-border tabular-nums"
                      data-testid={`value-gara-fisso-pezzi-${cat.type}`}
                    >
                      1
                    </td>
                    <EditableCell value={puntiVal} defaultValue={puntiDef} isOverridden={isOverridden(puntiPath)} onChange={v => updateValue(puntiPath, v)} onReset={() => resetValue(puntiPath)} testId={`input-gara-fisso-punti-${cat.type}`} step="0.25" />
                    <EditableCell value={euroVal} defaultValue={euroDef} isOverridden={isOverridden(euroPath)} onChange={v => updateValue(euroPath, v)} onReset={() => resetValue(euroPath)} testId={`input-gara-fisso-euro-${cat.type}`} />
                  </tr>
                );
              })}
            </tbody>
          ...
          <table className="w-full text-sm" data-testid="table-gara-fisso-gettoni">
            ...
            <tbody>
              {gettoniKeys.map(key => {
                ...
                return (
                  <tr key={key} className="even:bg-muted/30">
                    <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{GETTONI_LABELS[key]}</td>
      ...
      <Card>
        ...
        <CardContent><ScrollableTable>
          <table className="w-full text-sm" data-testid="table-gara-energia-pista">
            <thead>
              <tr className="bg-primary text-primary-foreground">
                <th className="p-2 text-left font-medium rounded-tl-md sticky left-0 bg-primary z-20">Soglia</th>
            ...
            <tbody>
              {pistaKeys.map(key => {
                ...
                return (
                  <tr key={key} className="even:bg-muted/30">
                    <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{key}</td>
      ...
      <Card>
        ...
        <CardContent><ScrollableTable>
          <table className="w-full text-sm" data-testid="table-gara-assicurazioni-punti">
            <thead>
              <tr className="bg-primary text-primary-foreground">
                <th className="p-2 text-left font-medium rounded-tl-md sticky left-0 bg-primary z-20">Prodotto</th>
                <th className="p-2 text-center font-medium rounded-tr-md w-32">Punti</th>
              </tr>
            </thead>
            <tbody>
              {ASSICURAZIONI_PRODUCT_KEYS.map(key => {
                ...
                return (
                  <tr key={key} className="even:bg-muted/30">
                    <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{ASSICURAZIONI_LABELS[key as keyof typeof ASSICURAZIONI_LABELS]}</td>
        ...
        <table className="w-full text-sm" data-testid="table-gara-protecta-gettoni">
          ...
          <tbody>
            {PROTECTA_PRODUCT_KEYS.map(key => {
              ...
              return (
                <tr key={key} className="even:bg-muted/30">
                  <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{PROTECTA_LABELS[key]}</td>
          ...
          <table className="w-full text-sm" data-testid="table-gara-extra-gara-punti">
            ...
            <tbody>
              {puntiKeys.map(key => {
                ...
                return (
                  <tr key={key} className="even:bg-muted/30">
                    <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{EXTRA_GARA_PUNTI_LABELS[key] || key}</td>
      ...
      <Card>
        ...
        <CardContent><ScrollableTable>
          <table className="w-full text-sm" data-testid="table-gara-extra-gara-soglie-multi">
            <thead>
              <tr className="bg-primary text-primary-foreground">
                <th className="p-2 text-left font-medium rounded-tl-md sticky left-0 bg-primary z-20">Tipo</th>
              ...
              {soglieKeys.map(bpKey => {
                ...
                return (
                  <tr key={bpKey} className="even:bg-muted/30">
                    <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{EXTRA_GARA_SOGLIE_LABELS[bpKey]}</td>
      ...
      <Card>
        ...
        <CardContent><ScrollableTable>
          <table className="w-full text-sm" data-testid="table-gara-extra-gara-premi">
            <thead>
              <tr className="bg-primary text-primary-foreground">
                <th className="p-2 text-left font-medium rounded-tl-md sticky left-0 bg-primary z-20">Cluster P.IVA</th>
            ...
            <tbody>
              {clusterKeys.map(cluster => (
                <tr key={cluster} className="even:bg-muted/30">
                  <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{CLUSTER_PIVA_LABELS[cluster] || cluster}</td>
                  {[0, 1, 2, 3, 4].map(i => {
        ...
        <Card>
          ...
          <CardContent><ScrollableTable>
            <table className="w-full text-sm" data-testid="table-gara-extra-gara-soglie-rs">
              <thead>
                <tr className="bg-primary text-primary-foreground">
                  <th className="p-2 text-left font-medium rounded-tl-md sticky left-0 bg-primary z-20">Ragione Sociale</th>
                ...
                {rsGroups.map(({ ragioneSociale, actualPdvCount, effectivePdvCount, isMultipos, computed, rsClusterPIva, defaultCluster }) => {
                  ...
                  return (
                    <tr key={ragioneSociale} className="even:bg-muted/30">
                      <td className="p-2 font-medium border border-border sticky left-0 z-10 bg-card [tr:nth-child(even)>&]:bg-[color-mix(in_srgb,hsl(var(--muted))_30%,hsl(var(--card)))]">{ragioneSociale}</td>
